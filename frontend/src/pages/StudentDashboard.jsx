...
import {
  Bell, ClipboardList, BookOpen, LogOut, Check,
  Upload, Trash2, Book, Video, Sun, Moon, Clock, FileText, X
} from 'lucide-react';
...
const SubmissionDropzone = ({ taskId, onFileSelected, selectedFile, onClear }) => {
  const { getRootProps, getInputProps, isDragActive, isDragReject } = useDropzone({
    onDrop: (accepted) => { if (accepted.length > 0) onFileSelected(accepted[0]); },
    accept: {
      'application/pdf': ['.pdf'],
    ...
    return (
      <div className="flex items-center gap-3 p-3 rounded-xl bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/30">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold text-indigo-800 dark:text-indigo-300 truncate">{selectedFile.name}</p>
          <p className="text-[10px] text-indigo-500 dark:text-indigo-400">{formatBytes(selectedFile.size)}</p>
        </div>
        <button
          type="button"
          onClick={onClear}
          className="p-1 rounded-md text-indigo-400 hover:text-indigo-600 dark:hover:text-indigo-200 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 transition-colors"
          title="Remove file"
        >
          <X size={14} />
        </button>
  ...
  return (
    ...
    >
      <input {...getInputProps()} id={`file-input-${taskId}`} />
      <Upload size={18} className={isDragActive ? 'text-indigo-500' : 'text-slate-400 dark:text-slate-500'} />
      <div className="text-center">
        <p className={`text-xs font-semibold ${isDragActive ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-600 dark:text-slate-400'}`}>
          {isDragReject ? 'File type not accepted' : isDragActive ? 'Drop to attach' : 'Drop file here or click to browse'}
        </p>
        <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">PDF, DOCX, JPEG, PNG, TXT</p>
      </div>
    </div>
  );
};
...
const StudentDashboard = ({ theme, toggleTheme }) => {
    ...
    const initializePortal = async () => {
      ...
      try {
        ...
        if (sRes.ok) {
          ...
          const subsMap = {};
          allSubs.filter(sub => sub.student?.email === userEmail)
                 .forEach(sub => { subsMap[sub.assignment?.id] = sub; });
  ...
  const handleRetractSubmission = async (assignmentId) => {
    if (!window.confirm('Retract your submission? You can re-upload a new file.')) return;
    ...
    try {
      ...
      setMySubmissions(prev => { const n = { ...prev }; delete n[assignmentId]; return n; });
  ...
  const pendingAssignments   = assignments.filter(t => !mySubmissions[t.id]);
  const completedAssignments = assignments.filter(t => !!mySubmissions[t.id]);
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-6 animate-fade-in">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm px-6 py-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center shrink-0 shadow-sm shadow-indigo-500/30">
              <span className="text-white font-bold text-base">{(profile?.firstName?.[0] ?? 'S').toUpperCase()}</span>
            </div>
            <div>
              <h1 className="text-base font-bold text-slate-900 dark:text-white">Welcome back, {profile?.firstName || 'Student'}</h1>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Your learning portal is live and synchronized</p>
            </div>
          </div>
          <div className="flex items-center gap-3 self-end sm:self-auto">
            <div className="px-3 py-1.5 rounded-lg bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 text-center">
              ...
              <p className="text-sm font-bold text-indigo-700 dark:text-indigo-300 font-mono leading-tight">{profile?.batch?.id || profile?.batchId || '—'}</p>
            </div>
            <div className="px-3 py-1.5 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-center">
              ...
              <p className="text-sm font-bold text-emerald-700 dark:text-emerald-300 leading-tight">{completedAssignments.length}/{assignments.length}</p>
            </div>
            <button onClick={toggleTheme} className="p-2 rounded-lg text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all" title="Toggle Theme">
              {theme === 'dark' ? <Sun size={16} /> : <Moon size={16} />}
            </button>
            <button onClick={() => { localStorage.clear(); navigate('/'); }} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700 hover:bg-rose-50 dark:hover:bg-rose-500/10 hover:text-rose-600 dark:hover:text-rose-400 hover:border-rose-200 dark:hover:border-rose-500/30 transition-all">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-5">
            ...
            {/* Assignments */}
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden flex flex-col h-[580px]">
              ...
              <div className="flex px-4 pt-3 gap-2 border-b border-slate-100 dark:border-slate-800 shrink-0">
                {[['pending', `Active (${pendingAssignments.length})`, 'indigo'], ['completed', `Done (${completedAssignments.length})`, 'emerald']].map(([tab, label, color]) => (
                  <button key={tab} onClick={() => setAssignmentTab(tab)}
                  ...
                  return (
                    <div key={task.id} className={[
                      ...
                      isSubmitted
                        ...
                        : isUrgent
                        ? 'bg-rose-50/30 dark:bg-rose-500/5 border-rose-300 dark:border-rose-500/40'
                      ...
                      <div className="flex items-start justify-between gap-2 mb-1.5">
                        ...
                        {isSubmitted ? (
                          ...
                          <span className={`shrink-0 inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium border ${isUrgent ? 'bg-rose-100 dark:bg-rose-500/20 text-rose-700 dark:text-rose-400 border-rose-300 dark:border-rose-500/30' : 'bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-200 dark:border-rose-500/20'}`}>
                      ...
                      {isSubmitted ? (
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex items-center gap-1.5 text-xs font-semibold text-emerald-600 dark:text-emerald-400">
                            <Check size={14} /> {isGraded ? 'Graded' : 'Submission secured'}
                          </div>
                          {!isGraded && (
                            <button onClick={() => handleRetractSubmission(task.id)} disabled={isRetracting}
                        ...
                        <form onSubmit={(e) => handleSubmitAssignment(e, task.id)} className="space-y-2">
                          <SubmissionDropzone
                            ...
                            onFileSelected={(file) => setSelectedFiles(prev => ({ ...prev, [task.id]: file }))}
                            onClear={() => setSelectedFiles(prev => { const n = { ...prev }; delete n[task.id]; return n; })}
                          />
                          <div className="flex gap-2">
                            <button type="button" onClick={() => { setActiveUploadId(null); setSelectedFiles(prev => { const n = { ...prev }; delete n[task.id]; return n; }); }}
                        ...
                        <button onClick={() => setActiveUploadId(task.id)}
                      ...
                      {submitStatus?.id === task.id && submitStatus?.status === 'error' && (
                        <p className="text-[11px] text-rose-500 dark:text-rose-400 mt-2 text-center font-medium">Action failed — please try again.</p>
                      )}
                    </div>
                  );
                })}
                  ...
                  <div key={mod.id} className="rounded-xl border border-slate-100 dark:border-slate-800 overflow-hidden bg-slate-50 dark:bg-slate-800/40 hover:border-slate-200 dark:hover:border-slate-700 transition-colors">
                    {embedUrl ? (
                      <div className="aspect-video w-full bg-black">
                        <iframe width="100%" height="100%" src={embedUrl} title={mod.title} frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />
                      </div>
                    ) : (
